use denoise::{
    ...
    denoise_fixed_point_time_embedding_padded::{
        ...
        build_padded_output_denoise_time_embedding_step_circuits,
        generate_padded_denoise_trace_with_computed_output, PaddedDenoiseShape,
    },
...
fn main() {
    ...
    let (z0, trace, expected_output) = generate_padded_denoise_trace_with_computed_output::<
        <E1 as Engine>::Scalar,
        N_MAX,
        TE_MAX,
        IN_MAX,
    ...
    println!("Final witness x_T = {:?}", final_witness);
    println!(
        "Final real output = {:?}",
        slice_real_vector::<N_REAL, N_MAX>(&expected_output)
    );
