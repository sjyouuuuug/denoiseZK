use denoise::{
    denoise_fixed_point_time_embedding::{
        compress_fixed_point_denoise_time_embedding_and_verify, generate_simple_time_table,
    ...
    denoise_fixed_point_time_embedding_padded::{
        build_padded_commitment_denoise_time_embedding_placeholder_circuit,
        build_padded_commitment_denoise_time_embedding_step_circuits,
...
fn main() {
    ...
    println!("Nova fixed-point denoise MLP + toy model commitment demo");
    ...
    let (z0, trace, hash_witnesses, expected_y, commitment) =
        generate_padded_denoise_trace_with_commitment::<
            <E1 as Engine>::Scalar,
            N_MAX,
            TE_MAX,
            IN_MAX,
            H_MAX,
        >(&public_params, x0_padded, shape);
    ...
    let placeholder = build_padded_commitment_denoise_time_embedding_placeholder_circuit::<
        N_MAX,
        TE_MAX,
        IN_MAX,
        H_MAX,
    ...
    let circuits = build_padded_commitment_denoise_time_embedding_step_circuits(
        &trace,
        &hash_witnesses,
        num_steps,
        num_iters_per_step,
