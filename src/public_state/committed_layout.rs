...
pub struct CommittedDenoiseStateLayout {
    pub n: usize,
    pub has_output: bool,
...
impl CommittedDenoiseStateLayout {
    pub fn new(
        n: usize,
        has_output: bool,
        total_iters: usize,
    ...
    pub fn assert_state_len(&self, len: usize) {
        assert_eq!(
            len,
            self.state_len(),
            "committed public state length mismatch: got {len}, expected {}",
            self.state_len()
        );
    }
