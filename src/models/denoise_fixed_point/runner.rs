...
use crate::{
    denoise_fixed_point::{FixedDenoiseIteration, PublicFixedPointDenoiseCircuit},
    fixed_point::FixedPointConfig,
    mlp_fixed_point_clipped_relu_lookup::FixedPointMlpClippedReluIteration,
    nova_ivc::{E1, E2, F1, G1, S1, S2},
};

pub fn build_fixed_point_denoise_placeholder_circuit<const N: usize, const H: usize>(
    total_iters: usize,
    num_iters_per_step: usize,
    config: FixedPointConfig,
) -> PublicFixedPointDenoiseCircuit<G1, N, H> {
    PublicFixedPointDenoiseCircuit {
        num_iters_per_step,
        total_iters,
        clipped_relu_table: config.clipped_relu_table(),
        config,
        seq: vec![
            FixedDenoiseIteration {
                x_i: [F1::ZERO; N],
                ...
                mlp_witness: FixedPointMlpClippedReluIteration {
                    x_i: [F1::ZERO; N],
                    hidden_raw_int: [0; H],
...
pub fn build_fixed_point_denoise_step_circuits<const N: usize, const H: usize>(
    trace: &[FixedDenoiseIteration<F1, N, H>],
    num_steps: usize,
    num_iters_per_step: usize,
    total_iters: usize,
    config: FixedPointConfig,
) -> Vec<PublicFixedPointDenoiseCircuit<G1, N, H>> {
    assert_eq!(
        trace.len(),
    ...
    (0..num_steps)
        .map(|i| PublicFixedPointDenoiseCircuit {
            num_iters_per_step,
            total_iters,
...
pub fn setup_fixed_point_denoise_public_params<const N: usize, const H: usize>(
    circuit: &PublicFixedPointDenoiseCircuit<G1, N, H>,
) -> PublicParams<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>> {
    PublicParams::<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>>::setup(
        circuit,
        &*S1::ck_floor(),
        &*S2::ck_floor(),
    )
...
pub fn run_fixed_point_denoise_recursive<const N: usize, const H: usize>(
    pp: &PublicParams<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>>,
    circuits: &[PublicFixedPointDenoiseCircuit<G1, N, H>],
    z0: &[F1],
) -> RecursiveSNARK<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>> {
    assert!(!circuits.is_empty(), "circuits must not be empty");
    let mut recursive_snark =
        RecursiveSNARK::<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>>::new(
            pp,
            &circuits[0],
            z0,
        )
...
pub fn verify_fixed_point_denoise_recursive<const N: usize, const H: usize>(
    recursive_snark: &RecursiveSNARK<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>>,
    pp: &PublicParams<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>>,
    num_steps: usize,
    z0: &[F1],
) {
...
pub fn compress_fixed_point_denoise_and_verify<const N: usize, const H: usize>(
    pp: &PublicParams<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>>,
    recursive_snark: &RecursiveSNARK<E1, E2, PublicFixedPointDenoiseCircuit<G1, N, H>>,
    num_steps: usize,
    z0: &[F1],
