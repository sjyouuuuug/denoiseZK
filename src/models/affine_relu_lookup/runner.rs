...
use crate::{
    affine_relu_lookup::{
        AffineReluLookupCircuit, AffineReluLookupIteration, AffineReluLookupParams,
    },
    nova_ivc::{E1, E2, F1, G1, S1, S2},
};

pub fn build_placeholder_circuit<const N: usize>(
    params: &AffineReluLookupParams<F1, N>,
    num_iters_per_step: usize,
) -> AffineReluLookupCircuit<G1, N> {
    AffineReluLookupCircuit {
        params: params.clone(),
        seq: vec![
            AffineReluLookupIteration {
                x_i: [F1::ZERO; N],
...
pub fn build_step_circuits<const N: usize>(
    params: &AffineReluLookupParams<F1, N>,
    trace: &[AffineReluLookupIteration<F1, N>],
    num_steps: usize,
    num_iters_per_step: usize,
) -> Vec<AffineReluLookupCircuit<G1, N>> {
    assert_eq!(
        trace.len(),
...
pub fn setup_public_params<const N: usize>(
    circuit: &AffineReluLookupCircuit<G1, N>,
) -> PublicParams<E1, E2, AffineReluLookupCircuit<G1, N>> {
    PublicParams::<E1, E2, AffineReluLookupCircuit<G1, N>>::setup(
        circuit,
        &*S1::ck_floor(),
        &*S2::ck_floor(),
    )
...
pub fn run_recursive<const N: usize>(
    pp: &PublicParams<E1, E2, AffineReluLookupCircuit<G1, N>>,
    circuits: &[AffineReluLookupCircuit<G1, N>],
    z0: &[F1],
) -> RecursiveSNARK<E1, E2, AffineReluLookupCircuit<G1, N>> {
    assert!(!circuits.is_empty(), "circuits must not be empty");

    let mut recursive_snark =
        RecursiveSNARK::<E1, E2, AffineReluLookupCircuit<G1, N>>::new(pp, &circuits[0], z0)
...
pub fn verify_recursive<const N: usize>(
    recursive_snark: &RecursiveSNARK<E1, E2, AffineReluLookupCircuit<G1, N>>,
    pp: &PublicParams<E1, E2, AffineReluLookupCircuit<G1, N>>,
    num_steps: usize,
    z0: &[F1],
) {
